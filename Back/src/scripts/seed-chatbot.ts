...
async function main() {
  ...
  try {
    ...
    await accounting.createExpense(ctx, USER_ID, {
      expenseDate: date(-2, 10),
      category: 'Rent',
      description: 'Office rent and utilities',
      vendorId: office.id,
      amount: 1800,
      taxAmount: 0,
      expenseAccountId: accountId('5300'),
      paymentMethod: 'bank_transfer',
    });
    await accounting.createExpense(ctx, USER_ID, {
      expenseDate: date(-1, 12),
      category: 'Software',
      description: 'Developer tools and subscriptions',
      vendorId: aws.id,
      amount: 950,
      taxAmount: 0,
      expenseAccountId: accountId('5100'),
      paymentMethod: 'card',
    });
    await accounting.createExpense(ctx, USER_ID, {
      expenseDate: date(0, 6),
      category: 'Travel',
