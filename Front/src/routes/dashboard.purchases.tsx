...
export const Route = createFileRoute("/dashboard/purchases")({ component: PurchasesRoute });

function PurchasesRoute() {
  const path = useRouterState({ select: (state) => state.location.pathname.replace(/\/+$/, "") });
  return path === "/dashboard/purchases" ? <PurchasesPage /> : <Outlet />;
}
...
function PurchasesPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      outstanding,
      paid: bills.filter((i) => i.status === "paid").reduce((sum, i) => sum + Number(i.total), 0),
      overdue: bills.filter((i) => i.status === "overdue").length,
      drafts: bills.filter((i) => i.status === "draft").length,
    };
  }, [bills]);
  ...
  return (
    <div className="space-y-5">
      <Header
        ...
        action={
          <Button className="bg-gradient-primary gap-1.5" onClick={() => navigate({ to: "/dashboard/purchases/manage" })}>
      ...
      <div className="bg-card border border-border-default rounded-2xl overflow-hidden shadow-soft">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border-default">
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="p-8 text-center text-on-surface-variant text-sm">{l("No bills found.")}</td></tr>
            ) : (
              filtered.map((b) => (
