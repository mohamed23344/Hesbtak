...
function Page() {
  ...
  return (
    <main dir={dir} className="min-h-screen bg-surface p-6 grid place-items-center">
      <div className="w-full max-w-3xl">
        ...
        <div className="bg-card border border-border-default rounded-2xl p-6">
          <h1 className="text-2xl font-bold">{l("Choose an organization")}</h1>

          <p className="text-sm text-on-surface-variant mt-1">
            {session?.tenants.length
              ? l("Select the financial workspace you want to open.")
              : l("No Available Organizations. Start by adding one or get an invitation.")}
          </p>
          ...
          <div className="grid sm:grid-cols-2 gap-3 mt-6">
            {(session?.tenants ?? []).map((tenant) => (
              <button
                ...
                onClick={() => select(tenant.organizationId)}
                className="text-start border border-border-default rounded-xl p-4 hover:border-primary"
              >
                <Building2 className="h-5 w-5 text-primary" />
                <p className="font-semibold mt-3">{tenant.organizationName}</p>
                <p className="text-xs text-on-surface-variant capitalize">
                  {l(tenant.role)} · {tenant.currency}
                </p>
              </button>
            ))}
          </div>
        ...
        {notifications.length > 0 && (
          <div className="bg-card border border-border-default rounded-2xl p-6 mt-4">
            ...
            <div className="divide-y divide-border-default mt-3">
              {notifications.slice(0, 10).map((notification) => (
                <div key={notification.id} className="py-3">
                  <p className="text-sm font-medium">{l(notification.title)}</p>
