...
function ExpenseVendorsPage() {
  ...
  useEffect(() => {
    void api<Vendor[]>("/tenant/vendors?billType=expense")
      .then(setVendors)
      .catch(() => toast.error("Could not load expense vendors"));
  }, []);
  ...
  const viewExpenses = async (vendor: Vendor) => {
    ...
    try {
      ...
      toast.error("Could not load vendor expenses");
    }
  };

  return (
    <div className="space-y-5">
      <Header title="Expense Vendors" desc="Vendors used on expense bills only." />
      <div className="overflow-hidden rounded-2xl border border-border-default bg-card shadow-soft">
        <table className="w-full text-sm">
          <thead className="bg-surface-container text-xs uppercase text-on-surface-variant">
            <tr>
              <th className="p-3 text-start font-medium">Vendor</th>
          ...
          <tbody className="divide-y divide-border-default">
            {vendors.length ? vendors.map((vendor) => (
              <tr key={vendor.id} className="hover:bg-surface-subtle">
                ...
                <td className="p-3">
                  <Button size="sm" variant="outline" onClick={() => void viewExpenses(vendor)}>View</Button>
                </td>
              </tr>
            )) : (
              <tr><td colSpan={4} className="p-8 text-center text-on-surface-variant">No vendors are linked to expense bills.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelected(null); }}>
        <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{selected?.name} expense bills</DialogTitle></DialogHeader>
          {!bills ? <p className="py-8 text-center text-sm text-on-surface-variant">Loading...</p> : (
            <div className="overflow-hidden rounded-xl border border-border-default">
              <table className="w-full text-sm">
                <thead className="bg-surface-container text-xs uppercase text-on-surface-variant">
                  <tr>
                    {["Bill", "Date", "Status", "Total", "Paid"].map((header) => (
                      <th key={header} className="p-3 text-start">{header}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-border-default">
                  {bills.length ? bills.map((bill) => (
                    ...
                    <tr><td colSpan={5} className="p-6 text-center text-on-surface-variant">No expense bills.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
