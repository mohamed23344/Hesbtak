...
export const Route = createFileRoute("/dashboard/expenses/returns")({ component: ExpenseReturns });
...
function ExpenseReturns() {
  const { t } = useI18n();
  const [returns, setReturns] = useState<Return[]>([]);

  const load = async () => {
    try {
      setReturns(await api<Return[]>("/tenant/purchase-returns"));
    } catch {
      setReturns([]);
    }
  };
  ...
  return (
    <div className="space-y-5">
      <Header title={t("returns")} desc={t("expensesDesc")} />
      ...
      <div className="bg-card border border-border-default rounded-2xl overflow-hidden shadow-soft">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border-default">
            {returns.length === 0 ? (
              <tr><td colSpan={5} className="p-8 text-center text-on-surface-variant text-sm">No expense returns yet.</td></tr>
            ) : (
              returns.map((r) => (
