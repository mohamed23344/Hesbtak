...
function ExpensesPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      unpaid,
      paid: expenses.filter((i) => i.status === "paid").reduce((sum, i) => sum + Number(i.total), 0),
      overdue: expenses.filter((i) => i.status === "overdue").length,
      thisMonth: expenses
        .filter((i) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid sm:grid-cols-4 gap-4">
        {[
          { l: t("outstanding"), v: money(stats.unpaid) },
          { l: t("paid30d"), v: money(stats.paid) },
          { l: t("overdue"), v: String(stats.overdue), alert: true },
          { l: t("totalThisMonth"), v: money(stats.thisMonth) },
        ].map((s) => (
