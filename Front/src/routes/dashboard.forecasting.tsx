...
function Page() {
  ...
  const todayLabel = firstForecast ? monthLabel(firstForecast.forecastMonth) : "";

  const chartData = useMemo(() => {
    const historical: ChartDatum[] = (forecast?.calculationDetails.monthlyActuals ?? []).slice(-4).map((item) => ({
      m: monthLabel(item.month),
    ...
    const forecastRows: ChartDatum[] = data.slice(0, 5).map((item, index) => ({
      m: monthLabel(item.forecastMonth),
  ...
  }, [data, forecast, confidenceBandRatio]);
        ...
        {[
          {
            ...
            sub: `${formatPercent(forecast?.calculationDetails.revenueGrowthRate ?? 0)} growth`,
            color: "#0F6E56",
          },
          {
            l: t("predictedExpenses"),
            v: money(firstForecast?.predictedExpense ?? 0),
            sub: `${formatPercent(forecast?.calculationDetails.expenseGrowthRate ?? 0)} growth`,
            color: "#993C1D",
          },
          {
            l: t("predictedCash"),
   ...
   <div className="bg-card border border-border-default rounded-lg p-5 shadow-soft">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-sm font-medium">Forecast - next 5 months</h3>
            {forecast && (
              <span className="text-[11px] px-2 py-0.5 rounded-md bg-[#FAEEDA] text-[#854F0B]">
                {forecast.confidence.score}% confidence
              </span>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-2 mb-3 text-xs text-on-surface-variant">
          <LegendItem color="#1D9E75" label="Revenue (actual)" />
          <LegendItem color="#1D9E75" label="Revenue (forecast)" dashed />
          <LegendItem color="#D85A30" label="Expenses (actual)" />
          <LegendItem color="#378ADD" label="Cash flow" block />
          <LegendItem color="#1D9E75" label="Confidence band" band />
          <span className="inline-flex items-center gap-1.5">
            <span className="h-3.5 w-0.5 rounded-full bg-[#888780]" /> Today
          </span>
        </div>
        ...
        <div className="h-80 relative">
          {!hasChartValues && (
            <div className="absolute inset-0 z-10 grid place-items-center text-sm text-on-surface-variant">
              No forecast values available yet.
            </div>
          )}
          <ForecastSvgChart
            data={chartData}
        ...
        {forecast && (
          <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 rounded-lg bg-surface-container px-4 py-3 text-xs text-on-surface-variant">
            <span><strong className="text-on-surface font-medium">Method:</strong> {forecast.method.revenue}</span>
            <span><strong className="text-on-surface font-medium">Periods:</strong> {forecast.calculationDetails.historicalPeriodCount} historical</span>
            <span><strong className="text-on-surface font-medium">Completeness:</strong> {Math.round(forecast.confidence.factors.dataCompleteness * 100)}%</span>
          </div>
        )}
      </div>
      {forecast && (
        <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-4">
          <div className="bg-card border border-border-default rounded-lg p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h3 className="font-semibold">Forecast audit trail</h3>
                <p className="text-sm text-on-surface-variant mt-1 max-w-2xl">
                  Deterministic formula forecast using only this tenant's financial history.
                </p>
              </div>
            ...
            <div className="grid md:grid-cols-3 gap-3 mt-5">
              <AuditMethod title="Revenue" method={forecast.method.revenue} rate={forecast.calculationDetails.revenueGrowthRate} />
              <AuditMethod title="Expenses" method={forecast.method.expenses} rate={forecast.calculationDetails.expenseGrowthRate} />
              <AuditMethod title="Cash flow" method={forecast.method.cashflow} rate={null} />
            </div>
            <div className="mt-4 rounded-lg bg-surface-container px-4 py-3 space-y-2">
              {forecast.formulaUsed.map((formula) => (
                <p key={formula} className="text-xs leading-relaxed text-on-surface-variant">{formula}</p>
              ))}
            </div>
          </div>
          ...
          <div className="bg-card border border-border-default rounded-lg p-5">
            <div className="flex items-center justify-between gap-3">
              <h3 className="font-semibold">Confidence basis</h3>
              <span className="text-xs px-2.5 py-1 rounded-md bg-surface-container">
                {forecast.confidence.score}% confidence
              </span>
            </div>
            <p className="text-sm text-on-surface-variant mt-2 leading-relaxed">{forecast.confidence.explanation}</p>
            <div className="space-y-3 mt-4">
              <ConfidenceRow label="Data availability" value={forecast.confidence.factors.historicalDataAvailability} />
              <ConfidenceRow label="Low variance" value={forecast.confidence.factors.historicalVariance} />
              <ConfidenceRow label="Seasonality" value={forecast.confidence.factors.seasonalConsistency} />
              <ConfidenceRow label="Completeness" value={forecast.confidence.factors.dataCompleteness} />
            </div>
            <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
              <span className="rounded-md bg-surface-container px-3 py-2">
                <strong className="font-medium text-on-surface">{forecast.calculationDetails.historicalPeriodCount}</strong> periods
              </span>
              <span className="rounded-md bg-surface-container px-3 py-2">
                <strong className="font-medium text-on-surface">{forecast.calculationDetails.forecastHorizonMonths}</strong> months forecast
              </span>
            </div>
          </div>
        </div>
...
function ForecastSvgChart({
  ...
  return (
    <svg className="h-full w-full overflow-visible" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Financial forecast chart">
      {ticks.map((tick) => {
        const y = scaleY(tick);
        return (
          <g key={tick}>
      ...
      {todayX !== null && (
        <g>
          ...
          <text x={todayX + 8} y={pad.top - 6} fontSize="11" fill="#888780">Today</text>
        </g>
      )}
...
function monthLabel(date: string) {
  return new Date(`${date.slice(0, 10)}T00:00:00.000Z`).toLocaleString(undefined, { month: "short" });
}
