  ...
  return (
    <div className="space-y-5">
        ...
        action={
          <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (v) loadVendorsAndBills(); }}>
            ...
            <DialogContent className="max-w-lg">
              <DialogHeader><DialogTitle>{l("Record Vendor Payment")}</DialogTitle></DialogHeader>
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input className="ps-9" placeholder={l("Search vendor...")} value={partySearch} onChange={(e) => setPartySearch(e.target.value)} />
                  </div>
                ...
                {form.vendorId && (
                  <div className="space-y-1.5">
                    <Label>{l("Bill (optional - select to pay specific bill)")}</Label>
                    ...
                    {selectedBill && (
                      <p className="text-xs text-on-surface-variant">
                        {l("Left amount")}: <span className="font-semibold text-on-surface">{money(selectedBillRemaining)}</span>
              ...
              <DialogFooter>
                ...
                <Button className="bg-gradient-primary" onClick={submit} disabled={!form.vendorId || !form.amount}>{l("Record")}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        }
      ...
      <div className="relative max-w-md">
        <Search className="h-4 w-4 absolute start-3 top-1/2 -translate-y-1/2 text-on-surface-variant" />
        <Input className="ps-9 bg-card" placeholder={l("Filter by vendor name...")} value={paymentSearch} onChange={(event) => setPaymentSearch(event.target.value)} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-surface-container text-on-surface-variant text-xs uppercase">
            <tr>
              ...
              <th className="text-start p-3 font-medium">{l("Bill")}</th>
